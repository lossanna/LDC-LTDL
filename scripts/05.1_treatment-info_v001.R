...
#   Look at dates of other prescribed burns
prescribed.burn <- treatment.info %>% 
  filter(Trt_Type_Sub == "Prescribed Burn") %>% 
  separate(Init_Date, into = c("init_m", "init_d", "init_y"), sep = "/", remove = FALSE) %>% 
  separate(Comp_Date, into = c("comp_m", "comp_d", "comp_y"), sep = "/", remove = FALSE)

#   See that most prescribed burns are completed the same year as initiated (or within 1 year)
pb.diffyr <- prescribed.burn %>% 
  filter(init_y != comp_y) %>% 
  mutate(diff_yr = as.numeric(comp_y) - as.numeric(init_y)) %>% 
  select(Trt_ID, Init_Date, init_y, Comp_Date, comp_y, diff_yr) %>% 
  arrange(diff_yr)
...
write_csv(treatment.info,
          file = "data/versions-from-R/05.1_Treatment-info_v001.csv")

write_csv(treatment.info.gisjoin,
          file = "data/versions-from-R/05.1_Treatment-info_v001-gisjoin.csv")
